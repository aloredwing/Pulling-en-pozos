...
st.write(
    "Carga el Excel de intervenciones para identificar qué pozos caen más veces a Pulling, "
    "por año, periodo, fecha, causa, motivo, falla, ubicación y causa final."
)
...
if archivo is not None:
    ...
    fecha_minima = base["Fecha_Inicio"].min().date()
    fecha_maxima = base["Fecha_Inicio"].max().date()
    ...
    st.success(
        f"Hoja leída: {hoja_usada}. Rango detectado: "
    ...
    servicios_disponibles = sorted(df_pulling["Servicio"].dropna().unique())
    ...
    baterias = ["Todas"] + sorted(df_pulling["Bateria"].dropna().unique())
    ...
    años_disponibles = sorted(df_pulling["Año"].dropna().unique())
    ...
    solo_repetidos_mismo_anio = st.sidebar.checkbox(
